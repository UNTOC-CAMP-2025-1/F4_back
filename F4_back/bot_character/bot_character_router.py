...
def assign_character_to_bot(bot_id: int, db: Session = Depends(get_db)):
...
def list_characters(bot_id: int, db: Session = Depends(get_db)):
...
def update_character(bot_id: int, request: BotCharacterUpdateRequest, db: Session = Depends(get_db)):
