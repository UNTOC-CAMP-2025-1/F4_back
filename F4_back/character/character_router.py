...
get_character_db = partial(get_db, domain="character")

@router.get("/", response_model=list[CharacterBase])
def get_characters(db: Session = Depends(get_character_db)):
...
def remove_character(character_id: int, db: Session = Depends(get_character_db)):
...
def get_character(character_id: int, db: Session = Depends(get_character_db)):
