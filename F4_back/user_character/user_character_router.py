...
get_usercharacter_db = partial(get_db, domain="user_character")

class CharacterRequest(BaseModel):
    character_id: int

@router.delete("/user_character/{character_id}", response_model=dict)
def delete_character(character_id: int, db: Session = Depends(get_usercharacter_db), user_id: int = Depends(get_current_user_id)):
...
def activate_character(character_id: int, db: Session = Depends(get_usercharacter_db), user_id: int = Depends(get_current_user_id)):
...
def list_characters(db: Session = Depends(get_usercharacter_db), user_id: int = Depends(get_current_user_id)):
...
def add_character(request: CharacterRequest, db: Session = Depends(get_usercharacter_db), user_id: int = Depends(get_current_user_id)):
